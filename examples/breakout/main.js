import { App, Synth, NOTE, IndexedDbStore, Button, Modal, KeyboardInput, drawText, fillRoundRect, drawCircle, fillRect, clamp, mulberry32, rollDie, } from "../../dist/index.js";
const PADDLE_W = 100;
const PADDLE_H = 14;
const BALL_R = 7;
const BRICK_COLS = 8;
...
class GameScene {
    ...
    draw(ctx, layout) {
        ...
        // paddle
        fillRoundRect(ctx, this.paddleX, layout.height - 40, PADDLE_W, PADDLE_H, 6, "#e2e8f0");
        // ball
        drawCircle(ctx, this.ballX, this.ballY, BALL_R, "#fbbf24");
        if (!this.launched && !this.gameOverModal) {
            drawText(ctx, "← →   to move    Space   to launch", layout.width / 2, layout.height - 80, { color: "#94a3b8", size: 14, align: "center", baseline: "middle" });
        }
        if (this.gameOverModal)
            this.gameOverModal.draw(ctx, layout);
    }
    onDown(x, y) {
        sound.ensureCtx();
        sound.resume();
        if (this.gameOverModal)
            this.gameOverModal.onDown(x, y);
    }
    onMove(x, y) {
        if (this.gameOverModal)
            this.gameOverModal.onMove(x, y);
    }
    onUp(x, y) {
        if (this.gameOverModal)
            this.gameOverModal.onUp(x, y);
    }
    onKeyDown(code) {
